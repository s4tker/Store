{{-- vista del producto con galeria precio y relacionados --}}
...
@section('content')
@php
    // prepara las imagenes de la galeria del producto
    ...
    // arma los datos que se mandan al carrito
    $cartProductPayload = [
        'id' => $Product->Id,
...
<div class="product-image-zoom relative overflow-hidden rounded-[1.25rem] bg-slate-50 cursor-zoom-in" data-product-zoom aria-label="vista ampliable del producto"
            >
                <img id="MainProductImage" src="{{ $mainImageUrl }}" alt="{{ $Product->Nombre }}" class="product-image-zoom-target h-[260px] w-full object-contain sm:h-[340px] lg:h-[460px]" draggable="false"
                >
            </div>
        </div>
...
@foreach($galleryImages as $image)
            ...
            @endphp
            <button type="button" class="rounded-[0.9rem] border border-slate-200 bg-white p-1.5 shadow-sm transition-colors hover:border-blue-300" onclick="SetMainProductImage('{{ $thumbUrl }}')"
            >
...
<div class="mt-3 flex flex-wrap gap-2">
                <span class="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1.5 text-[9px] font-black uppercase tracking-[0.12em] text-slate-600"> stock: {{ $Stock }}
                </span>
...
<div class="relative mt-2">
                    <p class="text-[12px] sm:text-[13px] leading-5 text-slate-600 transition-all"
...
<div x-show="!expanded" class="pointer-events-none absolute inset-x-0 bottom-0 h-8 bg-gradient-to-t from-slate-50 to-transparent"></div>
                ...
                @if($descriptionIsLong)
                <button type="button"
                    @click="expanded = !expanded" class="mt-2 text-[9px] font-black uppercase tracking-[0.14em] text-blue-600 transition-colors hover:text-slate-900" x-text="expanded ? 'ver menos' : 'ver más'"
                ></button>
                @endif
            </div>
            @endif
...
<div class="inline-flex items-center justify-between rounded-full border border-slate-200 bg-white px-2 py-1.5 sm:min-w-[130px]">
                ...
                <button type="button" onclick='AddCurrentProductToCart(@json($cartProductPayload))' class="btn-primary-es sm:w-auto px-5"
                > agregar al carrito
                </button>
            </div>
        </div>
...
{{-- lista de caracteristicas y valores del producto --}}
...
{{-- inicio de productos relacionados por categoria --}}
<section class="mt-10">
...
{{-- tarjetas de productos parecidos al actual --}}
