{{-- seccion del panel para categorias y subcategorias --}}
...
{{-- formulario que guarda categorias o subcategorias --}}

        <form id="FormAddCategoria" action="{{ route('admin.categorias.store') }}" method="POST" class="mt-6 space-y-4" data-store-url="{{ route('admin.categorias.store') }}" data-update-base="{{ url('/admin/categorias') }}"
        >
            @csrf
...
{{-- listado de categorias principales del catalogo --}}
...
{{-- tabla con categorias registradas --}}
...
<div class="flex items-center gap-3">
                            <button type="button" class="admin-action-link" data-edit-category="{{ $categoria->Id }}" data-category-name="{{ $categoria->Nombre }}" data-category-type="principal" data-category-parent=""
                            > Editar
                            </button>
                            <button type="button" class="admin-action-danger" data-delete-url="{{ route('admin.categorias.destroy', $categoria->Id) }}" data-delete-label="categoría {{ $categoria->Nombre }}"> Borrar
                            </button>
                        </div>
                    </div>
...
{{-- subcategorias asociadas a cada categoria principal --}}
...
<div class="flex items-center gap-3">
                                        <button type="button" class="admin-action-link" data-edit-category="{{ $subcategoria->Id }}" data-category-name="{{ $subcategoria->Nombre }}" data-category-type="subcategoria" data-category-parent="{{ $categoria->Id }}"
                                        > Editar
                                        </button>
                                        <button type="button" class="admin-action-danger" data-delete-url="{{ route('admin.categorias.destroy', $subcategoria->Id) }}" data-delete-label="subcategoría {{ $subcategoria->Nombre }}"> Borrar
                                        </button>
                                    </div>
                                </div>
                            @endforeach
