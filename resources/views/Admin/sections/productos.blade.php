{{-- seccion del panel para registrar y editar productos --}}
...
{{-- botones de categorias principales para filtrar productos --}}
@foreach($Categorias as $cat)
                <button type="button" class="admin-tab shrink-0 [&.is-active]:border-slate-900 [&.is-active]:bg-slate-900 [&.is-active]:text-white" data-root-category-id="{{ $cat->Id }}" data-root-category-name="{{ $cat->Nombre }}"
                >
                    {{ $cat->Nombre }}
                </button>
            @endforeach
...
{{-- formulario para registrar productos o editar los existentes --}}

        <form id="FormAddProducto" action="{{ route('admin.productos.store') }}" method="POST" enctype="multipart/form-data" class="mt-6 grid grid-cols-2 gap-4" data-store-url="{{ route('admin.productos.store') }}" data-update-base="{{ url('/admin/productos') }}"
        >
            @csrf
...
{{-- selector con marcas disponibles para el producto --}}
...
<div class="sm:col-span-2 xl:col-span-3">
                        ...
                        <select id="FilterCategory" class="admin-select">
                            <option value="">Todas</option>
{{-- selector de categorias para filtrar productos --}}
...
{{-- tarjetas de productos registrados en el catalogo --}}
@forelse($Productos as $producto)
                    ...
                    @endphp
                    <article class="product-card-admin admin-list-card group flex flex-col [&.is-active]:border-blue-300" data-product-id="{{ $producto->Id }}" data-product-name="{{ \Illuminate\Support\Str::lower($producto->Nombre) }}" data-product-brand="{{ \Illuminate\Support\Str::lower($producto->marca?->Nombre ?? '') }}" data-product-category="{{ \Illuminate\Support\Str::lower($categoryLabel) }}" data-product-root-category-id="{{ $producto->categoria?->ParentId ?: $producto->categoria?->Id }}" data-product-sku="{{ \Illuminate\Support\Str::lower($firstVariant?->Sku ?? '') }}"
                    >
...
<div class="admin-empty mt-4 hidden" id="NoProductResults"> No hay productos que coincidan con los filtros actuales.
            </div>
        </div>
    </section>
</div>
