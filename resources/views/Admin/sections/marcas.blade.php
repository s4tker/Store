{{-- seccion del panel para administrar marcas --}}
...
{{-- formulario que guarda marcas nuevas o editadas --}}

        <form id="FormAddMarca" action="{{ route('admin.marcas.store') }}" method="POST" class="mt-6 space-y-4" data-store-url="{{ route('admin.marcas.store') }}" data-update-base="{{ url('/admin/marcas') }}"
        >
            @csrf
...
<div class="flex items-center gap-3 sm:shrink-0">
                            <button type="button" class="admin-action-link" data-edit-brand="{{ $marca->Id }}" data-brand-name="{{ $marca->Nombre }}" title="Editar"
                            > Editar
                            </button>
                            <button type="button" class="admin-action-danger" data-delete-url="{{ route('admin.marcas.destroy', $marca->Id) }}" data-delete-label="marca {{ $marca->Nombre }}" title="Borrar"> Borrar
                            </button>
                        </div>
                    </div>
                </article>
