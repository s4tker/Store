{{-- vista del panel para revisar un pedido especifico --}}
...
<div class="min-w-0">
                        <a href="{{ route('admin.pedidos.index') }}" class="inline-flex items-center gap-2 text-sm text-slate-400 transition hover:text-slate-700">
                            <svg class="h-4 w-8 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 32 16" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M8 3 3 8l5 5M4 8h25"/></svg> Regresar a pedidos
                        </a>
...
<div class="rounded-2xl border border-emerald-100 bg-emerald-50/70 p-4">
                            <p class="text-[9px] font-black uppercase tracking-[0.18em] text-emerald-600">Teléfono</p>
                            <p class="mt-1 text-sm font-bold text-slate-900">{{ $pedido->usuario?->Telefono ?? 'Sin teléfono' }}</p>
                        </div>
...
<div class="mt-6 rounded-2xl border border-emerald-100 bg-emerald-50/60 p-5">
                        ...
                        <p class="mt-2 text-sm font-semibold leading-6 text-slate-700">
                            ...
                            <span class="block text-xs text-slate-500">
                                {{ collect([$pedido->direccion?->Ciudad, $pedido->direccion?->Region, $pedido->direccion?->Pais])->filter()->implode(' / ') ?: 'Sin ubicación' }}
                            </span>
...
{{-- selector con estados disponibles para el pedido --}}
