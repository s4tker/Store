{{-- vista del panel para filtrar y revisar pedidos --}}
...
{{-- filtros para buscar pedidos por estado y fecha --}}
...
{{-- selector con estados usados para filtrar pedidos --}}
...
{{-- selector de periodos para limitar la busqueda --}}
@foreach($PeriodosPedido as $periodo)
                                    <option value="{{ $periodo }}" @selected($FiltrosPedidos['periodo'] === $periodo)>
                                        @if($periodo === 'todos') Todo el historial
                                        @elseif($periodo === 'hora')
                                            Última hora
                                        @elseif($periodo === 'dia') Hoy
                                        @elseif($periodo === 'ayer') Ayer
                                        @elseif($periodo === 'semana') Esta semana
                                        @elseif($periodo === 'mes') Este mes
                                        @elseif($periodo === 'anio') Este año
                                        @else Personalizado
                                        @endif
                                    </option>
                                @endforeach
...
<div>
                        ...
                        <a href="{{ route('admin.pedidos.export', request()->only(['estado', 'periodo', 'desde', 'hasta'])) }}" class="admin-button h-[46px] px-6"> Exportar Excel
                        </a>
                    </div>

                    <p class="mt-3 text-xs font-semibold text-slate-400"> Para usar fechas u horas exactas, selecciona Personalizado y completa Desde / Hasta.
                    </p>
                </form>
...
{{-- tabla con pedidos registrados en el panel --}}
...
<div class="rounded-2xl border border-emerald-100 bg-emerald-50/70 p-4">
                                        <p class="text-[9px] font-black uppercase tracking-[0.18em] text-slate-400">Envío</p>
...
<div class="mt-5 flex flex-col justify-end gap-2 border-t border-slate-100 pt-5 sm:flex-row">
                                <a href="{{ route('admin.pedidos.export-one', $pedido->Id) }}" class="admin-button px-5 py-2.5 text-[11px] uppercase tracking-[0.16em]"
                                > Exportar
                                </a>
                                <a href="{{ route('admin.pedidos.show', $pedido->Id) }}" class="admin-button-primary px-5 py-2.5 text-[11px] uppercase tracking-[0.16em]"
                                > Detalles
                                </a>
                            </div>
                        </article>
