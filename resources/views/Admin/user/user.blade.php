{{-- vista para administrar usuarios y roles del sistema --}}
...
<div class="grid w-full max-w-xl gap-3 sm:grid-cols-2">
                        <x-admin.stat-card label="Administradores" :value="count($UsuariosAdmin)" tone="indigo">
                            <x-slot:icon>
...
<div class="relative">
                            ...
                            <input id="UserEmailSearch" type="search" class="admin-input pl-11" list="AdminUserEmails" placeholder="correo@dominio.com" autocomplete="off"
                            >
                        </div>
                        <datalist id="AdminUserEmails">
{{-- buscador por correo para encontrar usuarios existentes --}}
@foreach($UsuariosBusqueda as $usuario)
                                <option value="{{ $usuario['correo'] }}"></option>
                            @endforeach
                        </datalist>
                    </div>
{{-- formulario para registrar usuarios o cambiar sus roles --}}

                    <form id="FormAdminUsuario" action="{{ route('admin.usuarios.store') }}" method="POST" class="mt-7 space-y-5" data-store-url="{{ route('admin.usuarios.store') }}" data-update-base="{{ url('/admin/usuarios') }}"
                    >
                        @csrf
...
{{-- selector con roles que puede recibir el usuario --}}
...
@forelse($UsuariosAdmin as $usuario)
                            <article class="user-admin-item admin-list-card" data-load-user="{{ $usuario['id'] }}" data-user-email="{{ \Illuminate\Support\Str::lower($usuario['correo']) }}" data-user-role="{{ \Illuminate\Support\Str::lower($usuario['rol_nombre']) }}"
                            >
...
<div class="flex items-center gap-3">
                                        ...
                                        <button type="button" class="admin-action-danger" data-delete-url="{{ route('admin.usuarios.destroy', $usuario['id']) }}" data-delete-label="usuario {{ $usuario['correo'] }}"
                                        > Borrar
                                        </button>
                                    </div>
                                </div>
...
<div class="admin-empty mt-4 hidden" id="NoUserResults"> No hay administradores que coincidan con el filtro actual.
                    </div>
                </section>
            </div>
        </div>
