{{-- vista del perfil con datos direcciones y resumen de compra --}}
...
{{-- mensajes de validacion para direcciones del perfil --}}
...
<div class="bg-emerald-500 p-1 rounded-full mr-3 text-white">
        ...
        @endif
{{-- encabezado con nombre correo y accesos de cuenta --}}

        <header class="mb-12 relative">
<div class="absolute -left-4 top-0 w-1 h-full bg-blue-600 rounded-full"></div>
            <h1 class="text-3xl md:text-5xl font-black text-slate-900 tracking-tight uppercase italic leading-none"> Mi <span class="text-blue-600 not-italic">Cuenta</span>
            </h1>
            <p class="text-[11px] md:text-xs text-slate-400 font-black uppercase tracking-[0.3em] mt-4 flex items-center gap-2">
                <span class="w-8 h-[2px] bg-slate-200"></span> Gestión de perfil y logística
            </p>
        </header>
...
<div class="space-y-3">
                        <a href="{{ route('account.edit') }}" class="group flex items-center justify-center w-full bg-slate-900 hover:bg-blue-600 text-white font-black py-4 rounded-2xl text-[11px] uppercase tracking-widest transition-all shadow-lg shadow-slate-200"> Editar perfil
                        ...
                        <a href="{{ route('account.password') }}" class="block w-full bg-slate-50 hover:bg-slate-100 text-slate-600 font-bold py-4 rounded-2xl text-center text-[11px] uppercase tracking-widest transition-colors"> Seguridad
                        </a>
                        <a href="{{ route('logout') }}" class="block w-full border-2 border-red-50 hover:bg-red-50 text-red-500 font-black py-4 rounded-2xl text-center text-[11px] uppercase tracking-widest transition-all"> Cerrar Sesión
                        </a>
                    </div>
...
{{-- tarjetas con direcciones guardadas del usuario --}}
...
{{-- boton para borrar una direccion guardada --}}
...
<div class="absolute top-0 right-0 -mr-16 -mt-16 w-64 h-64 bg-blue-600/10 rounded-full blur-3xl"></div>
                    ...
                    <h3 class="text-xl font-black mb-8 flex items-center gap-3 italic uppercase">
                         <span class="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center not-italic text-sm"> + </span> Añadir Nueva Dirección
                    </h3>
{{-- formulario para agregar una direccion nueva --}}
...
<div class="space-y-2">
                                ...
                                <input type="text" name="Pais" value="{{ old('Pais') }}" placeholder="Ej. Perú" class="w-full p-4 bg-slate-800/50 border border-slate-700 rounded-2xl focus:border-blue-500 focus:ring-0 transition-all font-bold text-sm text-white placeholder-slate-600">
                            </div>
<div class="space-y-2">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Región / Estado *</label>
                                <input type="text" name="Region" value="{{ old('Region') }}" placeholder="Ej. Lima" class="w-full p-4 bg-slate-800/50 border border-slate-700 rounded-2xl focus:border-blue-500 focus:ring-0 transition-all font-bold text-sm text-white placeholder-slate-600">
                            </div>
                        </div>
<div class="grid grid-cols-1 md:grid-cols-2 gap-5">
<div class="space-y-2">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Ciudad *</label>
                                <input type="text" name="Ciudad" value="{{ old('Ciudad') }}" placeholder="Ej. Miraflores" class="w-full p-4 bg-slate-800/50 border border-slate-700 rounded-2xl focus:border-blue-500 focus:ring-0 transition-all font-bold text-sm text-white placeholder-slate-600">
                            </div>
<div class="space-y-2">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Código Postal / Referencia</label>
                                <input type="text" name="Referencia" value="{{ old('Referencia') }}" placeholder="Opcional" class="w-full p-4 bg-slate-800/50 border border-slate-700 rounded-2xl focus:border-blue-500 focus:ring-0 transition-all font-bold text-sm text-white placeholder-slate-600">
                            </div>
                        </div>
<div class="space-y-2">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Dirección Exacta *</label>
                            <input type="text" name="Direccion" value="{{ old('Direccion') }}" placeholder="Calle, número, departamento..." class="w-full p-4 bg-slate-800/50 border border-slate-700 rounded-2xl focus:border-blue-500 focus:ring-0 transition-all font-bold text-sm text-white placeholder-slate-600">
                        </div>

                        <button type="submit" class="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-5 rounded-2xl uppercase text-xs tracking-[0.2em] transition-all shadow-xl shadow-blue-600/20 active:scale-95 mt-4"> Guardar Dirección
                        </button>
                    </form>
                </div>
...
<div>
                    ...
                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">compra pendiente</p>
                </div>
...
<div class="flex items-center justify-between font-black uppercase text-[10px] mb-4 text-slate-400 tracking-tighter">
                ...
                <button class="w-full bg-slate-900 text-white font-black py-5 rounded-[1.5rem] opacity-40 cursor-not-allowed uppercase text-[10px] tracking-[0.2em] shadow-lg shadow-slate-200"> Compra no disponible
                </button>
            </div>
        </div>
    </aside>
