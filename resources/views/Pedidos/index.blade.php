{{-- vista con historial de pedidos del usuario --}}
...
<div class="max-w-5xl mx-auto space-y-10">
    ...
    <!-- encabezado del historial de pedidos -->
{{-- titulo y descripcion del historial de pedidos --}}
    <header class="flex flex-col md:flex-row md:items-end justify-between gap-6">
<div>
            <p class="text-[10px] font-black uppercase tracking-[0.3em] text-blue-600 mb-2">Historial</p>
            <h1 class="text-4xl md:text-5xl font-black text-slate-900 tracking-tight uppercase italic leading-none"> Mis <span class="text-slate-400">Pedidos</span>
            </h1>
    ...
    @if(session('success'))
<div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)" class="bg-emerald-50 border-l-4 border-emerald-500 p-5 rounded-2xl flex items-center justify-between shadow-sm">
<div class="flex items-center gap-3">
    ...
    <!-- tabla principal del historial de pedidos -->
    @if($pedidos->isEmpty())
...
<div class="bg-slate-50 w-20 h-20 rounded-3xl flex items-center justify-center mx-auto mb-6">
    ...
    @else
<div class="space-y-4">
            <!-- encabezados de la tabla visibles en escritorio -->
...
<div class="text-right">Acciones</div>
            ...
            <!-- filas con pedidos del usuario -->
{{-- filas con fecha estado total y acciones de cada pedido --}}
...
<div class="w-full md:w-auto">
                    ...
                    <!-- etiqueta visual con estado del pedido -->
<div class="w-full md:w-auto">
                        @php
                            $statusClasses = [
                    ...
                    <!-- monto total pagado o pendiente -->
<div class="w-full md:w-auto md:pr-4">
                    ...
                    <!-- botones para ver o cancelar pedido -->
<div class="flex flex-col sm:flex-row md:flex-nowrap items-stretch sm:items-center justify-end gap-2 w-full md:w-auto md:min-w-[12rem]">
                        <a href="{{ route('pedidos.show', $pedido->Id) }}" class="flex-1 md:flex-none text-center bg-slate-100 hover:bg-slate-200 text-slate-600 px-5 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all"> Detalles
                        </a>

                        @if($pedido->estado_normalizado === 'pendiente')
{{-- formulario para cancelar un pedido pendiente --}}
                            <form action="{{ route('pedidos.cancelar', $pedido->Id) }}" method="POST" onsubmit="return confirm('¿Seguro que deseas cancelar este pedido?')" class="flex-1 md:flex-none">
                                @csrf
                                <button type="submit" class="w-full bg-red-50 hover:bg-red-500 hover:text-white text-red-500 px-5 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all border border-red-100"> Cancelar
                                </button>
                            </form>
                        @endif
                    </div>
        ...
        <!-- paginacion del historial cuando hay muchos pedidos -->
<div class="mt-8">
            </div>
    @endif
</div>
...
@section('scripts')
<script>document.addEventListener('DOMContentLoaded', () => { localStorage.removeItem('electroshop-cart'); window.updateCartUI?.();
});
</script>
@endsection
@endif
