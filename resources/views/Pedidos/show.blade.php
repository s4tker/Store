{{-- vista del pedido con estado productos y cancelacion --}}
...
{{-- encabezado con numero estado y fecha del pedido --}}
    
    <header class="flex flex-col md:flex-row md:items-center justify-between gap-8 border-b border-slate-100 pb-10">
<div class="space-y-2">
<div class="flex items-center gap-3">
                <span class="px-3 py-1 bg-slate-900 text-white text-[9px] font-black uppercase tracking-[0.3em] rounded-md">Registro</span>
...
<div class="bg-white border border-slate-100 p-6 rounded-[2rem] shadow-sm">
                    <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Estado</p>
...
<div class="bg-white border border-slate-100 p-6 rounded-[2rem] shadow-sm">
                    <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Fecha</p>
...
<div class="relative z-10 space-y-6">
                    <h2 class="text-[10px] font-black uppercase tracking-[0.4em] text-blue-400">Datos de envío</h2>
<div class="space-y-2">
...
<div class="bg-white border border-slate-100 rounded-[3rem] p-8 md:p-10 shadow-xl shadow-slate-200/50">
                <h2 class="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 mb-10 border-b border-slate-50 pb-6 text-center">Productos</h2>
<div class="space-y-8">
...
<div class="space-y-1">
                                <h3 class="font-black text-slate-900 uppercase text-xs tracking-tight leading-tight group-hover:text-blue-600 transition-colors">
                                    {{ $detalle->variante?->producto?->Nombre ?? 'Producto #' . $detalle->VarianteId }}
                                </h3>
                                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest italic"> CANT. {{ $pedido->Cantidad }} <span class="mx-2">/</span> P.U: S/{{ number_format($detalle->Precio, 2) }}
                                </p>
                            </div>
<div class="text-right">
                                <p class="font-black text-slate-900 text-sm tracking-tighter"> S/{{ number_format($detalle->subtotal, 2) }}
                                </p>
                            </div>
                        </div>
                    @endforeach
...
<div class="flex justify-between items-center pt-4 border-t border-slate-50">
                        <span class="text-[11px] font-black text-slate-900 uppercase tracking-[0.3em]">Total</span>
...
<div class="mt-10 bg-slate-50 rounded-2xl p-4 flex items-center justify-center gap-3">
                    ...
                    <span class="text-[8px] font-black text-slate-400 uppercase tracking-[0.3em]">Garantía ElectroShop</span>
                </div>
            </div>
        </div>
    </div>
