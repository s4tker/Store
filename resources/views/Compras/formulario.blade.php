{{-- vista del formulario donde el usuario confirma la compra --}}
...
'CardImages' => [asset('img/cards/visa.png'),asset('img/cards/mastercard.png'),asset('img/cards/amex.png'),
],
];
@endphp
...
{{-- encabezado con titulo y resumen de compra --}}
...
<div class="absolute -left-4 top-0 w-1 h-full bg-blue-600 rounded-full"></div>
        <h1 class="text-3xl md:text-5xl font-black text-slate-900 tracking-tight uppercase italic leading-none"> Compra <span class="text-blue-600 not-italic">Seguro</span>
        </h1>
        <p class="text-[11px] md:text-xs text-slate-400 font-black uppercase tracking-[0.3em] mt-4 flex items-center gap-2">
            <span class="w-8 h-[2px] bg-slate-200"></span> Finaliza tu pedido
        </p>
    </header>
<div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
<div class="lg:col-span-8 space-y-8">
{{-- formulario que envia direccion comprobante y carrito del pedido --}}
...
{{-- mensajes de validacion que impiden continuar la compra --}}
...
{{-- selector de direcciones guardadas para el envio --}}
@foreach($DireccionesCompra as $direccion)
                        <label class="address-card p-5 border-2 rounded-[1.8rem] text-left transition-all cursor-pointer"
                            :class="Number(dirId) === {{ $direccion->Id }} ? 'is-active border-blue-500 bg-blue-50' : 'border-slate-100 hover:border-slate-200'">
                            <input type="radio" name="DireccionId" value="{{ $direccion->Id }}" x-model="dirId" class="sr-only" required
...
<div class="mt-6 flex flex-wrap items-center gap-3">
                        <a href="{{ route('account') }}" class="inline-flex items-center justify-center rounded-2xl bg-slate-900 px-5 py-3 text-[10px] font-black uppercase tracking-[0.2em] text-white transition-colors hover:bg-blue-600"> Cambiar dirección
                        </a>
                        <a href="{{ route('account') }}" class="inline-flex items-center justify-center rounded-2xl bg-slate-900 px-5 py-3 text-[10px] font-black uppercase tracking-[0.2em] text-white transition-colors hover:bg-blue-600"> Agregar dirección
                        </a>
                    </div>
                    @else
...
<div>
                            ...
                            <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Pago protegido</p>
                        </div>
                    </div>
...
<div>
                    ...
                    <span class="bg-blue-600/20 text-blue-400 px-4 py-1.5 rounded-full text-[10px] font-black uppercase" id="CompraItemCount"> 0 ITEMS
                    </span>
                </div>
<div id="CompraItems" class="CompraItems mb-8 max-h-80 overflow-y-auto pr-1 space-y-3 custom-scrollbar"></div>
